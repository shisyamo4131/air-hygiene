...
import HSimpleTableIndustrialPermission from '~/components/molecules/tables/HSimpleTableIndustrialPermission.vue'
...
export default {
  ...
  components: {
    HTemplateTabs,
    HBtnEdit,
    HSimpleTableIndustrialPermission,
    HBtnBack,
  },
  /******************************************************************
   * ASYNCDATA
...
<template>
  <h-template-tabs label="産業廃棄物処理業許可詳細" :tabs="tabs">
    <template #prepend-toolbar>
      <h-btn-back icon @click="$router.push(`/industrial-permissions/`)" />
    </template>
    <template #append-toolbar="{ tab }">
      <h-btn-edit
        v-show="tab === 1"
        icon
        @click="$router.push(`/industrial-permissions/${docId}/edit`)"
      />
    </template>
    <template #tab-0>
      <v-container fluid>
    ...
    <template #tab-1>
      <v-container fluid>
        <h-simple-table-industrial-permission v-bind="item" />
      </v-container>
    </template>
    <template #tab-2>
      <v-container fluid>
