<script>
/**
 * ### ASimpleTableMunicipalPermission
 *
 * @author shisyamo4131
 */
export default {
  ...
  props: {
    ...
    processingDiv: {
      type: String,
      default: 'delivery',
      validator: (v) => ['delivery', 'disposal'].includes(v),
      required: false,
    },
...
<template>
  <v-simple-table>
    <tbody>
      ...
      <tr>
        <td>処理区分</td>
        <td>{{ $PROCESSING_DIV[processingDiv] }}</td>
      </tr>
      <tr>
        <td>特別管理廃棄物</td>
        <td>{{ `${includeSpeciallyControled ? 'はい' : 'いいえ'}` }}</td>
      </tr>
      <tr>
        <td>許可日</td>
        <td>{{ dateOfPermission }}</td>
      </tr>
      <tr>
        <td>有効期限</td>
        <td>{{ dateOfExpiration }}</td>
      </tr>
    </tbody>
  </v-simple-table>
</template>
